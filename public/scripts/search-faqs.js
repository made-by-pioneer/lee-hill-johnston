...
window.onload = function () {
  ...
  request.open("GET", "/insurance-faqs.json", true);
...
function renderSearchResults(results) {
  ...
  if (results.length > 0) {
    results.forEach(function(result) {
      // Create result item
      var url_pre = resultDetails[result.ref].title;
      var url = url_pre.replace(/\s+/g, '-').toLowerCase().replace(/[?=]/g, "");
      url = url.replace(/\s+/g, '-').toLowerCase();
      var li = document.createElement('li');
      li.classList.add('card');
      li.innerHTML = '<div class="card-header"><a href="/faqs/#' + url + '">' + resultDetails[result.ref].title + '</a></div>';
      ul.appendChild(li);
    });
